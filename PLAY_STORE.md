...
2. Describe one core feature that requires the AccessibilityService API:
   - "Whitelister uses the AccessibilityService to read Instagram's on-screen UI only to
    detect when the user is viewing Reels and prevent scrolling between them, helping
    reduce time spent on Reels (digital wellbeing)."
